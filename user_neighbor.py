...
class UserNeighbor(object):
    ...
    def find_regions_categories_organizers(self):
        ...
        with open('event_info.txt') as f:
            for line in f:
                ...
                if result[1] in self.category_count_dict:
                    self.category_count_dict[result[4]] += 1
                else:
                    self.category_count_dict[result[4]] = 0
    ...
    def find_neighbor_on_cat(self):
        ...
        for item in self.social_influence_list:
            user_influence_on_categories[item[0]][self.event_location_cluster_org_cat_dict[item[0]][3]] \
                += item[2]
        ...
        for user in list(self.t_user_id_set):
            ...
            for i in xrange(self.top_n):
                self.neighbor_on_cat[user].add(sorted_similarity_list[i][1])
    ...
    def find_neighbor_on_org(self):
        ...
        for item in self.social_influence_list:
            user_influence_on_organizers[item[0]][self.event_location_cluster_org_cat_dict[item[0]][2]] \
                += item[2]
...
if __name__ == '__main__':
    ...
    neighbor_model.find_regions_categories_organizers()
